        ...
        <qgu-tree :datalist="treeData" :default-expand-all="defaultExpandAll" :show-include-child="false">
            <span slot-scope="{ node, data }" class="custom-tree-node">
                ...
                <span class="tree__option">
                    <el-tooltip
                        v-if="checkPermission('addTeacherCategory')"
                        class="item"
                        effect="dark"
                        :content="$t('新增类别')"
                        placement="top"
                    ...
                    <el-tooltip
                        v-if="checkPermission('updTeacherCategory')"
                        class="item"
                        effect="dark"
                        :content="$t('编辑')"
                        placement="top"
                    ...
                    <el-tooltip
                        v-if="checkPermission('delTeacherCategory')"
                        class="item"
                        effect="dark"
                        :content="$t('删除')"
                        placement="top"
                    ...
                    <el-tooltip
                        v-if="checkPermission('moveTeacherCategory')"
                        class="item"
                        effect="dark"
                        :content="$t('上移')"
                        placement="top"
...
<script>
    ...
    import {
        teacherCategory,
        teacherCategoryDelete,
        teacherCategoryInsert,
        teacherCategoryMove,
        teacherCategoryUpdate
    } from '@/api/teacher/teacherCategory';
    ...
    export default {
        ...
        methods: {
            checkPermission,
            getGroupTree() {
                teacherCategory(this.funCode).then(re => {
                ...
                this.$refs.editQuery.validate(valid => {
                    if (valid) {
                        if (this.editType === 'ADD') {
                            teacherCategoryInsert(this.editQuery).then(re => {
                        ...
                        teacherCategoryUpdate(this.editQuery).then(re => {
                            this.$store.dispatch('setAjaxIng', false);
                            const result = re.data;
                            if (result.success) {
            ...
            del(data) {
                this.$confirm(this.$t('此操作将永久删除该数据, 是否继续?'), {
                    ...
                    teacherCategoryDelete(params).then(re => {
                        const result = re.data;
                        if (result.success) {
            ...
            move(data, moveType) {
                ...
                teacherCategoryMove(params).then(re => {
                    const result = re.data;
                    if (result.success) {
