...
<template>
    <div v-show="!hide" class="login__container" :class="cls">
        <div class="bg__content" :style="{backgroundImage: `url(${bgImage})`}"/>
        <div class="login__info">
            <img class="login__logo__image" :src="kttxLogo">
            ...
            <div class="login__center">
                ...
                <div class="login__center--right">
                    <div class="login__form" :class="{'corpCode--hide': !isShowCorpCode}">
                        ...
                        <div v-if="isShowCorpCode && false" class="login__line" :class="{'line--error': errorMsg.corpCode}">
                            <input
                                id="corpCode"
...
<script>
    ...
    const kttxLogo = require('@/assets/images/kttx_logo.png');
    ...
    export default {
        ...
        async created() {
            ...
            this.logoImage = require('@/assets/images/short_logo.png');
        ...
        mounted() {
            const {query: {corpCode}} = this.$route;
            if (corpCode) {
                this.loginForm.corpCode = corpCode;
            }
        },
        destroyed() {
        },
        methods: {
            async shareGet() {
                const re = await shareGet();
                const result = re.data;
                if (!result.data) {
                    return;
                }
                const {copyright, logoFilePath, titleName, templateId, corpCode} = result.data;
                let {backgroundPath} = result.data;
                if (!backgroundPath) {
                    backgroundPath = this.BG_IMAGES[templateId];
                }
                this.bgImage = backgroundPath;
                this.logoImage = logoFilePath;
                this.companyName = titleName;
                this.copyrightText = copyright;
                this.cls = `type_${templateId}`;
                // this.loginForm.corpCode = corpCode;
                this.isShowCorpCode = false;
            },
            getTemplate() {
                this.cls = `type_${this.type}`;
            },
            checkRemember() {
