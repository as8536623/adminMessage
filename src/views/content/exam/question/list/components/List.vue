        ...
        <qgu-drag-row>
            ...
            <div slot="right" class="app__row--right">
                    ...
                    >
                        ...
                        <el-form-item :label="$t('难度') + '：'">
                            ...
                            >
                                <el-option label="全部" value/>
                                <el-option
                                    v-for="item in TEST_QUESTION_LEVEL_ENUM.arr"
                                    :key="item.code"
                                    :label="item.name"
                                    :value="item.code"
                                />
                        ...
                        <el-form-item :label="$t('知识点') + '：'">
                            <el-input
                                ref="knowledgeInput"
                                v-model.trim="listQuery.knowledge.knowledgeName"
                                :max-length="32"
                                :placeholder="$t('请选择知识点')"
                                clearable
                                @clear="knowledgeClear"
                                @focus="showKnowledgeSelect"
                            />
                        </el-form-item>
                ...
                <section class="opt__line veln__width">
                    <el-button
                        v-if="checkPermission('addItem')"
                        type="primary"
                        @click="add()"
                    >{{ $t('新增') }}
                    </el-button>
                    <el-button v-if="checkPermission('importItem')" @click="openUpload()">{{ $t('导入') }}</el-button>
                    <el-button
                    ...
                    <el-button v-if="checkPermission('delItem')" @click="batchDel()">{{ $t('删除') }}</el-button>
                    <router-link v-if="checkPermission('point')" :to="{name: 'knowledgePointsList'}">
                        <el-button class="knowledge__point__btn" type="text">{{ $t('知识点管理 >') }}</el-button>
                    </router-link>
                    <el-form
                        :inline="true"
                        :model="listQuery"
                        class="demo-form-inline pull-right"
                        style="overflow: hidden"
                    >
                        <search-input
                            v-model="listQuery.keyword"
                            :placeholder="$t('试题内容/编号/出题人')"
                            @enter="search"
                        />
                    </el-form>
                </section>
            ...
            const tableColumns = [
                ...
                },
                {
                    ...
                    render: (h, param) => {
                        ...
                        if (this.checkPermission('delItem')) {
