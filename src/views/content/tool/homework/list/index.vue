    ...
    <article class="app-container tool-container">
        <el-form :model="listQuery" :inline="true" class="demo-form-inline veln__width">
            <el-form-item :label="$t('项目状态') + '：'">
                <el-checkbox-group v-model="listQuery.statusArray" @change="handleCheckChange">
                    <el-checkbox
                        v-for="(item, index) in HOMEWORK_STATUS_ENUM.getAllValues()"
                        :key="index"
                        :label="item.code"
            ...
            <el-form-item class="pull-right">
                <search-input
                    ...
                    :placeholder="$t('请输入作业名称/编号')"
                    @enter="search"
                />
            </el-form-item>
            <el-form-item :label="$t('提交时间') + '：'" class="pull-right">
                <el-date-picker
                    v-model="signUpTime"
                    type="daterange"
                    format="yyyy-MM-dd"
        ...
        <section class="opt__line">
            <el-button v-if="checkPermission('addWork')" type="primary" @click="add()">{{ $t('新增') }}</el-button>
            <el-button v-if="checkPermission('delWork')" @click="batchDel()">{{ $t('删除') }}</el-button>
            <el-checkbox
                v-model="listQuery.isOnlyMyself"
                class="pull-right only__my"
                @change="isOnlyMyselfChange"
            >{{ $t('只看我创建的作业') }}</el-checkbox>
        </section>
        <qgu-table
...
<script>
    import {HOMEWORK_STATUS_ENUM, HOMEWORK_TYPE_ENUM} from '@/enum';
    ...
    export default {
            ...
            const tableColumns = [
                {
                    ...
                    disabled: () => {
                        return (
                            this.checkPermission('addWork') ||
                            this.checkPermission('delWork')
                        );
                    }
                },
                {
                ...
                },
                {
                    ...
                    sortable: 'custom',
                    render: (h, param) => {
                        const startTime = param.row.submitSt;
                        const endTime = param.row.submitEt;
                ...
                },
                {
                    ...
                    minWidth: 160,
                    render: (h, param) => {
                        return this.HOMEWORK_TYPE_ENUM.getName(param.row.workType);
                    }
                },
                {
                    ...
                    minWidth: 160,
                    render: (h, param) => {
                        return param.row.createUserName
                            ? param.row.createUserName
                            : '--';
                    }
                },
                {
                    ...
                    render: (h, param) => {
                        return param.row.status
                            ? this.HOMEWORK_STATUS_ENUM.getName(param.row.status)
                            : '--';
                    }
                },
                {
                    ...
                    render: (h, param) => {
                        ...
                        switch (param.row.status) {
                            case 'DRAFT':
                                if (this.checkPermission('addWork')) {
                                    optArr.push(editBtn);
                                    optArr.push(startBtn);
                                }

                                if (this.checkPermission('delWork')) {
        ...
        methods: {
            ...
            getList(condition) {
                ...
                params.submitSt =
                    null !== this.signUpTime && this.signUpTime.length
                        ? `${this.signUpTime[0]} 00:00:00`
                        : null;
                params.submitEt =
                    null !== this.signUpTime && this.signUpTime.length
                        ? `${this.signUpTime[1]} 23:59:59`
                        : null;
                params.statusArray = params.statusArray.join(',');
                this.listLoading = true;
                getWorkPage(params).then(re => {
            ...
            add() {
                this.$router.push({name: 'homeworkBaseInfo'});
            },
            edit(data) {
                this.$router.push({
            ...
            batchDel() {
                ...
                this.$confirm(this.$t('此操作将永久删除该数据, 是否继续?'), {
                    ...
                    this.deleteHomeworks(selectedIds);
                });
            },
            deleteHomeworks(params) {
                deleteHomework(params).then(re => {
                    const result = re.data;
                    if (result.success) {
            ...
            disable(data) {
                this.$confirm(this.$t('确认停用该数据吗, 是否继续?'), {
                    ...
                    discontinuedWork(data.id).then(re => {
                        const result = re.data;
                        if (result.success) {
            ...
            qrCode(data) {
                ...
                let businessUrl = `#/homework/detail?id=${data.id}`;
