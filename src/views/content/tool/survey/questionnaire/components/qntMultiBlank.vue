...
<template>
    <div class="qnt__multi__blank">
        ...
        <draggable
            tag="ul"
            class="qnt__multi__blank__itemList"
            :item-list="val.itemList"
            handle=".qnt__item__sort"
            v-bind="dragOptions"
        >
            <li v-for="(item, idx) in val.itemList" :key="idx" class="qnt__multi__blank__item">
                ...
                />
                <div v-if="item.imgUrls.length" class="qnt__answer__imgs">
                    <div v-for="(img, i) in item.imgUrls" :key="i" class="qnt__img__wrapper">
                        <img :src="img.fileUrl" alt="IMAGE" class="qnt__img">
                        <div class="qnt__img__opt">
                            <div class="qnt__remove__wrapper" @click="removeAnswerImg(idx, i)">
                                <svg-icon icon-class="close_o" class-name="qnt__remove"/>
                                <div class="qnt__remove__text">{{ $t('删除') }}</div>
                            </div>
                        </div>
                ...
                <el-input class="qnt__blank__npt multi" disabled/>
            </li>
        </draggable>
...
<script>
    ...
    export default {
        ...
        methods: {
            addItem() {
                this.val.itemList.push(deepClone(this.multiBlankTemp));
            },
            remove(index) {
