...
<template>
    <article class="app-container tool-container">
        <el-form :model="listQuery" :inline="true" class="demo-form-inline veln__width">
            ...
            <el-form-item class="pull-right">
                <search-input
                    ...
                    :placeholder="$t('请输入签到标题/编号')"
                    @enter="search"
                />
            </el-form-item>
            <el-form-item :label="$t('开始时间') + '：'" class="pull-right">
                <el-date-picker
                    v-model="signInTime"
                    type="daterange"
                    format="yyyy-MM-dd"
        ...
        <section class="opt__line">
            <el-button v-if="checkPermission('addSign')" type="primary" @click="add()">{{ $t('新增') }}</el-button>
            <el-button v-if="checkPermission('delSign')" @click="batchDel()">{{ $t('删除') }}</el-button>
        </section>
        <qgu-table
...
<script>
    import {SIGN_STATUS_ENUM} from '@/enum';
    ...
    export default {
            ...
            const tableColumns = [
                {
                    ...
                    disabled: () => {
                        return (
                            this.checkPermission('addSign') ||
                            this.checkPermission('delSign')
                        );
                    }
                },
                {
                ...
                },
                {
                    ...
                    render: (h, param) => {
                        ...
                        return startTime
                            ? moment(param.row.startTime).format('YYYY-MM-DD HH:mm')
                            : '';
                    }
                },
                {
                ...
                },
                {
                    ...
                    render: (h, param) => {
                        return param.row.status
                            ? this.signStatus.getName(param.row.status)
                            : '--';
                    }
                },
                {
                    ...
                    render: (h, param) => {
                        ...
                        switch (param.row.status) {
                            case 'DRAFT':
                                if (this.checkPermission('updSign')) {
                                    optArr.push(editBtn);
                                }
                                if (this.checkPermission('enableSign')) {
                                    optArr.push(startBtn);
                                }
                                if (this.checkPermission('delSign')) {
                            ...
                            case 'NOT_STARTED':
                                ...
                                //     optArr.push(deleteBtn);
                                // }
                                if (this.checkPermission('disableSign')) {
        ...
        methods: {
            ...
            getList(condition) {
                ...
                params.startTime =
                    null !== this.signInTime && this.signInTime.length
                        ? moment(this.signInTime[0]).format('YYYY-MM-DD HH:mm:ss')
                        : null;
                params.endTime =
                    null !== this.signInTime && this.signInTime.length
                        ? moment(this.signInTime[1]).format('YYYY-MM-DD HH:mm:ss')
                        : null;
            ...
            qrCode(data) {
                ...
                let businessUrl = `#/sign/detail?id=${data.id}`;
